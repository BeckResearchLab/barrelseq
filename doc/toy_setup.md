...
./shim.py sample add --config-file $PROJECT_DIR/config.yaml --name sample1 --group group1 --fastq-files /tmp/fq1 /tmp/fq2 --description 'first sample' 
./shim.py sample add --config-file $PROJECT_DIR/config.yaml --name sample2 --group group2 --fastq-files /tmp/fq1 /tmp/fq2 --description 'second sample'
```
